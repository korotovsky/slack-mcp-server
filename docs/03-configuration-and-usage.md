...
Saved items tools (`saved_list`, `saved_add`, `saved_delete`, `saved_update`, `saved_clear_completed`) manage the "Save for Later" panel and are **registered by default**, but only when authenticating with browser session tokens (`xoxc`/`xoxd`) — Slack's `saved.*` endpoints are not available to OAuth (`xoxp`) or bot (`xoxb`) tokens, so the tools are not registered for those.
