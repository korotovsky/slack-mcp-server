...
func NewMCPServer(provider *provider.ApiProvider, logger *zap.Logger) *MCPServer {
	...
	s.AddTool(mcp.NewTool("conversations_upload_file",
		mcp.WithDescription("Upload a local file to a Slack channel or DM. Supports optional initial comment, thread reply, and custom title. Disabled by default - set SLACK_MCP_UPLOAD_FILE_TOOL env var to enable (true/1, comma-separated channel IDs, or '!CID' negation list)."),
		...
		mcp.WithString("initial_comment",
			mcp.Description("Optional caption/message posted with the file."),
		),
		mcp.WithString("thread_ts",
			mcp.Description("Optional thread parent timestamp to reply into a thread, format 1234567890.123456."),
		),
		mcp.WithString("title",
			mcp.Description("Optional display title. Defaults to the filename."),
		),
	), conversationsHandler.ConversationsUploadFileHandler)

	s.AddTool(mcp.NewTool("conversations_download_files",
		mcp.WithDescription("Download all files attached to a Slack message to a local directory. Returns a JSON array of saved file metadata and paths."),
		mcp.WithTitleAnnotation("Download Message Files"),
		mcp.WithReadOnlyHintAnnotation(true),
		mcp.WithString("channel_id",
			mcp.Required(),
			mcp.Description("ID of the channel in format Cxxxxxxxxxx or its name starting with #... or @... aka #general or @username_dm."),
		...
		mcp.WithString("message_ts",
			mcp.Required(),
			mcp.Description("Timestamp of the message with attached files, format 1234567890.123456."),
		),
		mcp.WithString("output_dir",
			mcp.Description("Optional absolute directory to save files into. Defaults to /tmp/slack-files/<message_ts>/."),
		),
